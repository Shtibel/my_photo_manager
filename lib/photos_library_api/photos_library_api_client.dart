...
class PhotosLibraryApiClient {
  ...
  Future<MediaItemsResponse> allMediaItems(String token) async {
    print('<-- http');
    String _url = 'https://photoslibrary.googleapis.com/v1/mediaItems?'
        'pageSize=100'
        '&access_token=$token';
    ...
    return http.get(_url).then(
      (Response response) {
        ...
        return MediaItemsResponse.fromJson(jsonDecode(response.body));
      },
    );
  }
